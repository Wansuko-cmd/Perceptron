...
pub fn exp_d1(x: &GPUBuffer, result: &GPUBuffer, context: &Context) {
    math(&context.pipeline.math.exp_d1, "exp_d1", x, 0f32, result, context);
}

pub fn ln_d1(x: &GPUBuffer, e: f32, result: &GPUBuffer, context: &Context) {
    math(&context.pipeline.math.ln_d1, "ln_d1", x,e, result, context);
}

pub fn sigmoid_d1(x: &GPUBuffer, result: &GPUBuffer, context: &Context) {
    math(&context.pipeline.math.sigmoid_d1, "sigmoid_d1", x, 0f32, result, context);
}

pub fn pow_d1(x: &GPUBuffer, n: i32, result: &GPUBuffer, context: &Context) {
    math(&context.pipeline.math.pow_d1, "pow_d1", x,n as f32, result, context);
}

pub fn sqrt_d1(x: &GPUBuffer, e: f32, result: &GPUBuffer, context: &Context) {
    math(&context.pipeline.math.sqrt_d1, "sqrt_d1", x,e, result, context);
}
...
fn math(
    ...
    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some(label),
        layout: &context.pipeline.math.bind_group_layout,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
