...
#[unsafe(no_mangle)]
pub extern "C" fn com_wsr_cpu_plus_d0_to_d1(x: f32, y: *const CPUBuffer, result: *mut CPUBuffer) {
    let y = unsafe { &*y };
    let result = unsafe { &mut *result };
    plus::plus_with_d0_to_d1(x, y, result);
}

#[unsafe(no_mangle)]
pub extern "C" fn com_wsr_cpu_plus_d1_to_d0(x: *const CPUBuffer, y: f32, result: *mut CPUBuffer) {
    let x = unsafe { &*x };
    let result = unsafe { &mut *result };
    plus::plus_with_d1_to_d0(x, y, result);
}

#[unsafe(no_mangle)]
pub extern "C" fn com_wsr_cpu_plus_d1_to_d1(x: *const CPUBuffer, y: *const CPUBuffer, result: *mut CPUBuffer) {
    let x = unsafe { &*x };
    let y = unsafe { &*y };
    let result = unsafe { &mut *result };
