...
inline void zipWithD1ToD2(
    ...
    if (axis == 0) {
        #ifdef _OPENMP
        #pragma omp parallel for collapse(2) if(yi * yj >= PARALLEL_THRESHOLD)
        #endif
        for (int i = 0; i < yi; i++) {
...
inline void zipWithD1ToD3(
    ...
    if (axis == 0) {
        #ifdef _OPENMP
        #pragma omp parallel for collapse(3) if(yi * yj * yk >= PARALLEL_THRESHOLD)
        #endif
        for (int i = 0; i < yi; i++) {
            float x_value = x_ptr[i];
            for (int j = 0; j < yj; j++) {
...
inline void zipWithD3ToD4(
    ...
    if (axis1 == 0 && axis2 == 1 && axis3 == 2) {
        #ifdef _OPENMP
        #pragma omp parallel for collapse(4) if(yi * yj * yk * yl >= PARALLEL_THRESHOLD)
        #endif
        for (int i = 0; i < yi; i++) {
            for (int j = 0; j < yj; j++) {
                for (int k = 0; k < yk; k++) {
