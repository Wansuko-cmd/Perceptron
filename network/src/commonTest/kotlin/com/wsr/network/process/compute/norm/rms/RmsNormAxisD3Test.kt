...
class RmsNormAxisD3Test {
    val target0 get() = RmsNormAxisD3(outputX = 2, outputY = 2, outputZ = 3, axis = 0, e = 1e-6f)
    val target1 get() = RmsNormAxisD3(outputX = 2, outputY = 2, outputZ = 3, axis = 1, e = 1e-6f)
    val target2 get() = RmsNormAxisD3(outputX = 2, outputY = 2, outputZ = 3, axis = 2, e = 1e-6f)
    val input
        get() = batchOf(
            IOType.d3(
                IOType.d2(
    ...
    fun `Axis0_train=axis0で正規化および勾配を伝播`() = networkTestRule {
        ...
        assertEquals(
            expected = IOType.d1(0.0000f, 0.0312f, 0.0468f),
            actual = actual[1][0][1],
            absoluteTolerance = 1e-4f,
        )
        assertEquals(
        ...
        assertEquals(
            expected = IOType.d1(0.0312f, 0.0000f, -0.0312f),
            actual = actual[1][1][1],
            absoluteTolerance = 1e-4f,
        )
    }
    ...
    fun `Axis2_train=axis2で正規化および勾配を伝播`() = networkTestRule {
        ...
        assertEquals(
            expected = IOType.d1(-0.0625f, -0.0625f, -0.1250f),
            actual = actual[1][1][1],
            absoluteTolerance = 1e-4f,
        )
    }
