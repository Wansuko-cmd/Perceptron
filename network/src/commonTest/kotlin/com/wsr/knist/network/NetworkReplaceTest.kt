...
class NetworkReplaceTest {
    ...
    private fun createD1Network(): Network<Batch<IOType.D1>, Batch<IOType.D1>> = NetworkBuilder
        .inputD1(converter = RawD1(3), optimizer = optimizer, initializer = Fixed(0.5f))
        .affine(neuron = 4, id = "mid")
        .affine(neuron = 2, id = "last")
        .meanSquare()

    private fun createD2Network(): Network<Batch<IOType.D2>, Batch<IOType.D2>> = NetworkBuilder
        .inputD2(converter = RawD2(2, 3), optimizer = optimizer, initializer = Fixed(0.5f))
        .affine(neuron = 4, id = "mid")
        .meanSquare()

    private fun createD3Network(): Network<Batch<IOType.D3>, Batch<IOType.D1>> = NetworkBuilder
        .inputD3(converter = RawD3(2, 2, 2), optimizer = optimizer, initializer = Fixed(0.5f))
        .scale(id = "mid")
        .reshapeToD1()
        .meanSquare()
    ...
    fun `replace_ComputeD1=層をDSLで差し替えられる`() = networkTestRule {
        ...
        val expected = NetworkBuilder
            .inputD1(converter = RawD1(3), optimizer = optimizer, initializer = Fixed(0.5f))
            .affine(neuron = 4, initializer = Fixed(1f))
            .affine(neuron = 2)
            .meanSquare()
     ...
     */
    private fun createReshapeNetworkA(): Network<Batch<IOType.D1>, Batch<IOType.D1>> = NetworkBuilder
        .inputD1(converter = RawD1(6), optimizer = optimizer, initializer = Fixed(0.5f))
        .reshapeToD2(i = 2, j = 3, id = "d1tod2")
        .reshapeToD3(i = 1, j = 2, k = 3, id = "d2tod3")
        .reshapeToD1(id = "d3tod1")
        .meanSquare()

    private fun createReshapeNetworkB(): Network<Batch<IOType.D1>, Batch<IOType.D1>> = NetworkBuilder
        .inputD1(converter = RawD1(8), optimizer = optimizer, initializer = Fixed(0.5f))
     ...
     */
    @Test
    fun `replaceInput=入力Converterを差し替えられる`() = networkTestRule {
        val original = createD1Network()
        val replaced = original.replaceInput(RawD1(3))
    ...
    @Test
    fun `replaceOutput=出力Converterを差し替えられる`() = networkTestRule {
        val original = createD1Network()
        val replaced = original.replaceOutput(RawD1(2))
    ...
    fun `replace=空ブロックで入出力形状が同じ層は削除できる`() = networkTestRule {
        val original = NetworkBuilder
            .inputD1(converter = RawD1(3), optimizer = optimizer, initializer = Fixed(0.5f))
            .bias(initializer = Fixed(1f), id = "del")
            .meanSquare()
