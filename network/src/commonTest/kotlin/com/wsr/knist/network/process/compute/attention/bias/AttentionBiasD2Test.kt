...
class AttentionBiasD2Test {
    ...
    private val dummyContext get() = Context(Batch.of(IOType.d1(0f, 0f, 0f)))

    @Test
    fun `Causal=queryより後ろのkey位置に-1e9を全ヘッド・全クエリ共通で加える`() = networkScopeTestRule {
        val bias = AttentionBiasD2.Causal(inputI = 3)

        val actual = with(bias) { forward(scaled, dummyContext) }
    ...
    fun `Mask=指定した値のトークンに対応するkey位置に-1e9を全ヘッド・全クエリ共通で加える`() = networkScopeTestRule {
        val bias = AttentionBiasD2.Mask(value = 0f)
        // 3番目のトークンだけpadding(0)扱いになる
        val context = Context(Batch.of(IOType.d1(1f, 1f, 0f)))

        val actual = with(bias) { forward(scaled, context) }
