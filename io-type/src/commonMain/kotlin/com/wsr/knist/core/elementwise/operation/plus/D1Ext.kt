...
@ScopeOp
operator fun IOType.D1.plus(other: Float): IOType.D1.Global {
    val result = Backend.plus(x = value, y = other)
    return IOType.D1.Global(value = result)
}

@ScopeOp
operator fun IOType.D1.plus(other: IOType.D0): IOType.D1.Global {
    val result = Backend.plus(x = value, xi = 1, xj = size, y = other.value, axis = 0)
    return IOType.D1.Global(value = result)
}

@ScopeOp
operator fun IOType.D1.plus(other: IOType.D1): IOType.D1.Global {
    val result = Backend.plus(x = value, y = other.value)
    return IOType.D1.Global(value = result)
}

@ScopeOp
fun IOType.D1.plus(other: IOType.D2, axis: Int): IOType.D2.Global {
