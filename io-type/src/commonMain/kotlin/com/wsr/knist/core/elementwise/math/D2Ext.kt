...
@ScopeOp
fun IOType.D2.exp(): IOType.D2.Global {
    val result = Backend.exp(x = value)
    return IOType.D2(shape = shape, value = result)
}

@ScopeOp
fun IOType.D2.ln(e: Float): IOType.D2.Global {
    val result = Backend.ln(x = value, e = e)
    return IOType.D2(shape = shape, value = result)
}

@ScopeOp
fun IOType.D2.pow(n: Int): IOType.D2.Global {
    val result = Backend.pow(x = value, n = n)
    return IOType.D2(shape = shape, value = result)
}

@ScopeOp
fun IOType.D2.softmax(): IOType.D2.Global {
    val max = max()
...
@ScopeOp
fun IOType.D2.softmax(axis: Int): IOType.D2.Global {
...
@ScopeOp
fun IOType.D2.sqrt(@ScopeOpDefault("1e-7f") e: Float = 1e-7f): IOType.D2.Global {
