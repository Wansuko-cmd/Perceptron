...
@ScopeOp
operator fun IOType.D2.times(other: Float): IOType.D2.Global {
    val result = Backend.times(x = value, y = other)
    return IOType.D2.Global(shape = shape, value = result)
}

@ScopeOp
operator fun IOType.D2.times(other: IOType.D0): IOType.D2.Global {
    val result = Backend.times(x = value, xi = 1, xj = size, y = other.value, axis = 0)
    return IOType.D2.Global(shape = shape, value = result)
}

@ScopeOp
fun IOType.D2.times(other: IOType.D1, axis: Int): IOType.D2.Global {
    val result = Backend.times(x = value, xi = i, xj = j, y = other.value, axis = axis)
    return IOType.D2.Global(shape = shape, value = result)
}

@ScopeOp
operator fun IOType.D2.times(other: IOType.D2): IOType.D2.Global {
    val result = Backend.times(x = value, y = other.value)
    return IOType.D2.Global(shape = shape, value = result)
}

@ScopeOp
fun IOType.D2.times(other: IOType.D3, axis1: Int, axis2: Int): IOType.D3.Global {
