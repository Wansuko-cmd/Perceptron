...
class ReshapeD3ToD2Test {
    ...
    fun `ReshapeD3ToD2の_expect=D3をD2に平坦化`() {
        ...
        assertEquals(expected = 3.0, actual = output[0, 2])
        assertEquals(expected = 4.0, actual = output[0, 3])
        assertEquals(expected = 5.0, actual = output[1, 0])
        assertEquals(expected = 6.0, actual = output[1, 1])
        assertEquals(expected = 7.0, actual = output[1, 2])
        assertEquals(expected = 8.0, actual = output[1, 3])
    }
    ...
    fun `ReshapeD3ToD2の_train=D2のdeltaをD3に戻す`() {
        ...
        val calcDelta: (List<IOType>) -> List<IOType> = {
            listOf(IOType.d2(2, 4) { x, y -> (x * 4 + y + 1) * 2.0 })
        }
