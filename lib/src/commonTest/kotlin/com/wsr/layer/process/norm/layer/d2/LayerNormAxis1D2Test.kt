    ...
    fun `LayerNormAxis1D2の数値微分テスト=dxが計算されて返される`() {
        ...
        for (i in 0 until 2) {
            ...
            for (j in 0 until 2) {
                ...
                val outputPlus = norm._expect(batchOf(IOType.d2(listOf(2, 2), inputPlus.toList())), context) as Batch<IOType.D2>
                val lossPlus = calcLoss(outputPlus, calcDelta)

                // input[i, j]を少し減らす
                val inputMinus = input[0].value.copyOf()
                inputMinus[i * 2 + j] -= epsilon
                val outputMinus = norm._expect(batchOf(IOType.d2(listOf(2, 2), inputMinus.toList())), context) as Batch<IOType.D2>
