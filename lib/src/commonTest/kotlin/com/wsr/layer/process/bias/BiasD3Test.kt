...
class BiasD3Test {
    ...
    fun `BiasD3の_train=deltaをそのまま返し、バイアスを更新`() {
        ...
        // output = input + 0.9f
        assertEquals(expected = 1.9f, actual = afterOutput[0, 0, 0], absoluteTolerance = 1e-6f)
        assertEquals(expected = 2.9f, actual = afterOutput[0, 0, 1], absoluteTolerance = 1e-6f)
    }
    ...
    fun `BiasD3の_train=バイアスが更新され、期待通りの出力になる`() {
        ...
        assertEquals(expected = 9.0f, actual = afterOutput[1, 0, 0], absoluteTolerance = 1e-6f)
        assertEquals(expected = 10.8f, actual = afterOutput[1, 0, 1], absoluteTolerance = 1e-6f)
        assertEquals(expected = 12.6f, actual = afterOutput[1, 1, 0], absoluteTolerance = 1e-6f)
        assertEquals(expected = 14.4f, actual = afterOutput[1, 1, 1], absoluteTolerance = 1e-6f)
    }
}
