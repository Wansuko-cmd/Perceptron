...
class MinMaxNormD3Test {
    ...
    fun `MinMaxNormD3の_train=alphaが更新され、期待通りの出力になる`() {
        ...
        assertEquals(expected = 1.110204081632653f, actual = afterOutput[1, 0, 0], absoluteTolerance = 1e-6f)
        assertEquals(
            expected = 1.3775510204081634f,
            actual = afterOutput[1, 0, 1],
            absoluteTolerance = 1e-6f,
        )
        assertEquals(
            expected = 1.6408163265306124f,
            actual = afterOutput[1, 1, 0],
            absoluteTolerance = 1e-6f,
        )
        assertEquals(expected = 1.9f, actual = afterOutput[1, 1, 1], absoluteTolerance = 1e-6f)
    }
}
