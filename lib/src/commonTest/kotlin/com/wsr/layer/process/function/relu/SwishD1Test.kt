...
class SwishD1Test {
    ...
    fun `SwishD1の_expect=swish関数を適用`() {
        ...
        assertEquals(expected = 0.0 * sig0, actual = output[0], absoluteTolerance = 1e-4)
        assertEquals(expected = 1.0 * sig1, actual = output[1], absoluteTolerance = 1e-4)
        assertEquals(expected = 2.0 * sig2, actual = output[2], absoluteTolerance = 1e-4)
    }
