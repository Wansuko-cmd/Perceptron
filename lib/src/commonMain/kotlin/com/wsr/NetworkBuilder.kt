...
sealed interface NetworkBuilder<I, O : IOType> {
    ...
    data class D1<I> internal constructor(
        val inputSize: Int,
        override val inputConverter: Converter,
    ...
    companion object {
        fun inputD1(inputSize: Int, optimizer: Optimizer, seed: Int? = null) = D1<IOType.D1>(
            ...
            inputConverter = LinearD1(inputSize),
            layers = emptyList(),
        )

        fun <T> inputD1(inputConverter: Converter.D1<T>, optimizer: Optimizer, seed: Int? = null) = D1<T>(
        ...
        fun inputD2(x: Int, y: Int, optimizer: Optimizer, seed: Int? = null) = D2<IOType.D2>(
            ...
            optimizer = optimizer,
            random = seed?.let { Random(it) } ?: Random,
            inputConverter = LinearD2(x, y),
            layers = emptyList(),
        )

        fun <T> inputD2(inputConverter: Converter.D2<T>, optimizer: Optimizer, seed: Int? = null) = D2<T>(
        ...
        fun inputD3(x: Int, y: Int, z: Int, optimizer: Optimizer, seed: Int? = null) = D3<IOType.D3>(
            ...
            optimizer = optimizer,
            random = seed?.let { Random(it) } ?: Random,
            inputConverter = LinearD3(x, y, z),
            layers = emptyList(),
        )

        fun <T> inputD3(inputConverter: Converter.D3<T>, optimizer: Optimizer, seed: Int? = null) = D3<T>(
            inputX = inputConverter.outputX,
            inputY = inputConverter.outputY,
